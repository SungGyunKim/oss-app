...
`app.requestSingleInstanceLock()`으로 앱 중복 실행을 방지한다.

- 이미 실행 중인 상태에서 다시 실행하면 기존 앱의 메인 창을 포커스
- 로그인 상태면 메인 창 표시, 비로그인이면 로그인 창 표시

## 앱 실행 흐름

1. 앱 시작 → 싱글 인스턴스 확인 → 창 표시 + 트레이 상주
2. 첫 설치 시 자동 실행(openAtLogin) 활성화 (이후 사용자가 직접 토글)
3. 로그인 상태 확인 → `osstem_token` 쿠키 존재 여부로 판별 (domain: `.denall.com` 또는 `.osstem.com`)
...
로그인 완료 후 `{MEMBER_ORIGIN}/api/mem/member/getProfile` API를 호출해 사용자 정보를 조회한다.
...
사업자(BIZMN) 계정이 같은 사업자의 다른 직원이 보낸 메시지를 수신하면 토스트 알림을 억제한다.

```
조건: isBusiness() && 발신자.memDvCd === 'BIZMN' && 발신자.custId === 로그인 사용자.customerId
→ 토스트 표시하지 않음
```
...
- **작업 표시줄 깜빡임**: 메시지 수신 시 대상 창을 3회 깜빡인 후 강조 상태 유지
  - 전용 대화방이 있지만 포커스 아닌 경우 → 해당 대화방 창을 깜빡임
  - 전용 대화방이 없는 경우 → main 창을 생성/표시하고 깜빡임
  - main 창 포커스 시 또는 토스트 클릭으로 대화방 열릴 때 → 깜빡임 해제
...
`userData/settings.json` 파일로 앱 설정을 관리한다. 파일이 없으면 기본값으로 자동 생성된다.
...
interface Settings {
  general: {
    autoLaunchInitialized: boolean // 자동 실행 초기화 완료 여부
  }
  notification: {
...
- 첫 설치 후 최초 실행 시 `openAtLogin: true`를 자동으로 설정하여 Windows 시작 시 앱이 자동 실행됨
- `autoLaunchInitialized` 플래그로 최초 1회만 설정하고, 이후에는 사용자가 환경설정에서 직접 토글
- Electron `app.setLoginItemSettings({ openAtLogin })` API 사용
...
- [x] ~~**로그아웃 후 쿠키 처리**~~ — auth.ts에서 로그아웃 URL 호출 + 명시적 쿠키 삭제 구현 완료
- [x] ~~**IPC 이벤트 전체 명세**~~ — session-expired, open-post-room, get/set-auto-launch, get/update-settings 구현 완료
- [x] ~~**창 기본 크기 / 최소 크기**~~ — config.ts WINDOW_CONFIG에 정의 완료 (메인 1200x800/최소 800x600, 로그인 480x720, 채팅 500x700, 토스트 360x75, 설정 640x440)
